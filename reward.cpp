...
void Reward::normalize(int reward_max, int dal_max, double factor)
{
    if (arguments.mcs_method == RL_DAL)
        this->normalized_reward = factor * this->rl_component / reward_max + (1 - factor) * this->dal_component / dal_max;
    else
        this->normalized_reward = factor * this->ll_component / reward_max + (1 - factor) * this->dal_component / dal_max;
}
